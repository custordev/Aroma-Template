...
export default async function SimilarProducts({ id }: { id: string }) {
  ...
  const allSimilarProducts: ProductProps[] = products.filter(
    (product: any) => product.id !== id
  );
  const similarProducts = allSimilarProducts.slice(1, 5);
  // console.log(similarProducts);
  return (
    <div className="flex items-center justify-center flex-col gap-16 py-4">
      <h2 className="font-bold text-start text-4xl">SimilarProducts</h2>
